...
#ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null  $DEV "mkdir -p $HOST_DEPLOY_PATH_BUCKET"
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null  $DEV "echo 'test ssh'"
...
scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null  "$BUCKET_COMMIT" $DEV:$HOST_DEPLOY_PATH_BUCKET
...
# deploy bucket
php7.4 ./vendor/bin/dep deploy-bucket $DEV \
...
then
  ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null  $DEV "cd $HOST_DEPLOY_PATH/release/magento/ && /bin/bash $HOST_DEPLOY_PATH/deployer/scripts/staging/release_setup.sh"
fi
...
# deploy release
php7.4 ./vendor/bin/dep $DEFAULT_DEPLOYER $DEV \
-o bucket-commit=$BUCKET_COMMIT \
-o host_bucket_path=$HOST_DEPLOY_PATH_BUCKET \
-o deploy_path_custom=$HOST_DEPLOY_PATH \
-o deploy_keep_releases=$INPUT_KEEP_RELEASES \
...
if [ -d "$PROJECT_PATH/magento" ]
then
  ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null  $DEV "cd $HOST_DEPLOY_PATH/current/magento/ && /bin/bash $HOST_DEPLOY_PATH/deployer/scripts/staging/post_release_setup.sh"
fi

# Run pwa-studio post release script if the directory exists
if [ -d "$PROJECT_PATH/pwa-studio" ]
then
 ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null  $DEV "cd $HOST_DEPLOY_PATH/current/pwa-studio/ && /bin/bash $HOST_DEPLOY_PATH/deployer/scripts/staging/post_release_setup_pwa_studio.sh"
fi

ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null  $DEV "cd $HOST_DEPLOY_PATH_BUCKET && /bin/bash $HOST_DEPLOY_PATH/deployer/scripts/staging/post_release_cleanup.sh $INPUT_KEEP_RELEASES"
